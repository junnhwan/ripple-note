...
- **多场景 Feed 与复合游标分页**：实现最新流、热门流、关注流、标签流等 Feed 场景，采用 `published_at + id`、`hot_score + id` 等复合游标分页方式，避免 offset 分页在无限滚动下的重复、漏刷和深分页性能问题；Feed 查询只返回 `published + public` 内容，登录态通过批量 hydration 回填点赞、收藏、关注状态，避免 N+1 查询。
